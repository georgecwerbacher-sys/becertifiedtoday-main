...
import { isEncorPortalProduct, isSecplusPortalProduct } from "../server-lib/ccna-portal-stripe.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      note:
        "Emails from Stripe checkout (CCNA, ENCOR, and Security+ portal access). One row per email per product.",
    });
  } catch (err) {
