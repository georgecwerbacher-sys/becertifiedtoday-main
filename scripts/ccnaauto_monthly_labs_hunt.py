#!/usr/bin/env python3
"""Monthly CCNAAUTO 200-901 labs / sim / PBQ workflow: collect → compare → save (.md).

Scans competitor pages for Python script, Ansible playbook, REST API, and drag-and-drop signals.
Compares against CCNA automation overlap labs and future CCNAAUTO bank pages.
"""
...
SOURCING = ROOT / "data" / "ccnaauto-question-sourcing" / "labs"
RUNS_DIR = SOURCING / "runs"
CONFIG = ROOT / "data" / "ccnaauto-question-sourcing" / "config" / "ccnaauto-labs-sources.json"
...
def write_compare_report(
    ...
    lines = [
        ...
        "---",
        "",
        f"# CCNAAUTO 200-901 labs / sim compare — {run_id}",
        "",
...
def write_net_new_md(
    ...
    lines = [
        ...
        "---",
        "",
        f"# CCNAAUTO 200-901 labs / sim net-new — {run_id}",
        "",
    ...
    if not net_new:
        ...
        for i, r in enumerate(net_new, 1):
            ...
            lines.append("- [ ] Draft lab/sim in CCNAAUTO bank or CCNA automation track")
            lines.append("")
            lines.append("---")
            lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="CCNAAUTO 200-901 labs/sim monthly: collect → compare → save (.md)",
    )
