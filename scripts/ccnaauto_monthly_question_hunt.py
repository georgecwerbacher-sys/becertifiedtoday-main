#!/usr/bin/env python3
"""Monthly CCNAAUTO 200-901 question workflow: collect → compare → save (.md).

  collect  — competitor poll + optional import CSV (does NOT read BCT)
  compare  — match collected rows against CCNA automation overlap bank
...
SOURCING = ROOT / "data" / "ccnaauto-question-sourcing"
RUNS_DIR = SOURCING / "runs"
CONFIG = SOURCING / "config" / "ccnaauto-web-sources.json"
...
USER_AGENT = "BCT-CCNAAUTO-200-901-Monthly-Hunt/1.0 (+https://becertifiedtoday.com; research)"
...
def load_compare_slugs(cfg: dict | None = None) -> set[str]:
    """CCNA questions that overlap CCNAAUTO automation / programmability objectives."""
    product_cfg = cfg or load_source_config()
    bct = product_cfg.get("bct_compare") or {}
    prefixes = tuple(bct.get("objective_prefixes") or ["6."])
    keywords = tuple(bct.get("slug_keywords") or [])
    slugs: set[str] = set()
    if not TOPIC_MAP.is_file():
        return slugs
    data = json.loads(TOPIC_MAP.read_text(encoding="utf-8"))
    for filename, objs in (data.get("assignments") or {}).items():
        slug = filename.replace(".html", "")
        obj_ids = [str(o) for o in objs]
        if any(obj_id.startswith(prefixes) for obj_id in obj_ids):
            slugs.add(slug)
            continue
        if keywords and any(kw in slug for kw in keywords):
            slugs.add(slug)
    return slugs
...
def load_bct_stems() -> list[dict]:
    ...
    compare_slugs = load_compare_slugs()
    if compare_slugs:
        items = [it for it in items if it["slug"] in compare_slugs]
    return items
...
def write_compare_report(
    ...
    lines = [
        ...
        "---",
        "",
        f"# CCNAAUTO 200-901 compare — {run_id}",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="CCNAAUTO 200-901 monthly: collect → compare → save (.md)",
    )
    ...
    for name, help_text in (
        ...
        ("compare", "Step 2 — compare to CCNA automation overlap bank"),
