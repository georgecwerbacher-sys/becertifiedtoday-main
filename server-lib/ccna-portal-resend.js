...
export async function sendSecplusPortalMagicEmail({ to, magicUrl }) {
  const key = (process.env.RESEND_API_KEY || "").trim();
  if (!key) {
    console.warn("[secplus-portal] RESEND_API_KEY unset — skipping magic-link email");
    return false;
  }
  ...
  const subject =
    (process.env.RESEND_SECPLUS_PORTAL_SUBJECT || "").trim() ||
    "Your CompTIA Security+ training portal link";
...
<p>This link stops working when your paid access window ends. While access is still active, you can request another email from the Security+ portal restore page.</p>`,
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    console.error("[secplus-portal] Resend error:", res.status, text.slice(0, 500));
    throw new Error("Resend rejected email: HTTP " + res.status);
  }
  return true;
}
