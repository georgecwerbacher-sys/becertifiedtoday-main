...
|---------|----------------------------|-------------------------|
| Security+ SY0-701 | $9.99 sim · **$24.99 portal list** · **ONETIMEDEAL** ($7 off → $17.99 one-time launch popup) | Email after free 35-min sim |
...
| 3 | **Promotion codes** → Create code **`ONETIMEDEAL`** linked to that coupon |
...
| 6 | Test: `{payment_link_url}?prefilled_promo_code=ONETIMEDEAL` → total **$17.99** |

**Math:** $24.99 − $7.00 = **$17.99** (one-time launch deal).

Site behavior: launch popup open → checkout URL includes `prefilled_promo_code=ONETIMEDEAL`. Popup closed → full **$24.99** link, no code.

**If your Stripe promotion code is not `ONETIMEDEAL`**, set `LAUNCH_PROMO_CODE` in `public/COMP_TIA_SEC+/js/secplus-portal-checkout.js` to match.
...
| 2026-06-04 | **ONETIMEDEAL** on-site only (launch popup); list **$24.99** → **$17.99** with coupon | Coupon not in Search RSAs; prefilled when popup open |
