---
type: campaign-ops
product: secplus
tags:
  - marketing
  - google-ads
  - budget
  - promo
---

# Google Ads spend — $500 → $1,000 credit promo

**Offer:** Spend **$500** before **July 19, 2026** → **$1,000** ad credit (confirm terms in Google Ads → Promotions).

**Buffer target:** **$510** cumulative eligible spend.

**Tracker:** [[google-ads-spend-promo-2026.csv]] — billing through **2026-06-24** · overview actuals **2026-06-25**.

**Overview export:** [[google-ads-overview-2026-06-25/README.txt|google-ads-overview-2026-06-25/]]

---

## Progress snapshot (through Jun 25, 2026)

| Metric | Amount |
|--------|-------:|
| **Account campaign spend (cumulative)** | **$280.26** |
| Sec+ campaigns only (all names) | **$147.29** (includes Jun 25 **$24.58**) |
| CCNA wedge lab | $122.07 |
| Other (CCNA_2, ENCOR-TOP, CCNA_Study_Portal) | $10.90 |
| Taxes & fees (may not count toward promo) | $0.18 |
| **Remaining to $500** | **$219.74** |
| **Remaining to $510** | **$229.74** |

**Promo progress:** **56%** of $500 · **55%** of $510 buffer.

You are **well ahead** of where a Jun 24 Sec+-only launch would be — earlier **SEC+** / **Security+** runs and **ccna_wedge_lab** spend already count toward account spend (verify Google’s promo widget uses the same scope).

---

## Jun 24–25 — live Sec+ PBQ campaign

| | |
|---|---|
| Campaign | `Security+ SY0-701 · Exam prep · becertifiedtoday` |
| Jun 24 spend | **$9.63** · **4 clicks** · ~**$2.41** avg CPC |
| **Jun 25 spend** | **$24.58** · **12 clicks** · **215 impr** · ~**$2.05** avg CPC · [[google-ads-overview-2026-06-25/README.txt\|overview export]] |
| Jun 23 | Launch evening — **no separate billing line** (likely rolled into Jun 24) |

**ccna_wedge_lab:** last spend **Jun 21** ($1.49) — appears **paused**; if it stays off, forward spend ≈ Sec+ only.

---

## Revised budget advice — **keep $15/day**

Earlier math assumed **~$0** spent by Jun 24. With **$255.68** already in:

| Daily budget (Sec+ only, ccna paused) | Cross **$500** | Cross **$510** | By Jul 19 at $15/day |
|--------------------------------------|----------------|----------------|----------------------|
| **$15/day** | ~**Jul 11** | ~**Jul 12** | **$630** cumulative ✓ |
| $20/day | ~Jul 4 | ~Jul 5 | $755 ✓ |

**Recommendation:** **Stay at $15/day** for the 21-day conversion test. You should **clear $500 with room to spare** before Jul 19 without raising budget.

Only bump to **$20/day** if Google’s promo tracker shows **ineligible spend** (e.g. old campaigns excluded) and you fall behind their progress bar.

---

## Spend by period (account campaigns)

| Period | Spend | Notes |
|--------|------:|-------|
| May 21–31 | $15.41 | CCNA_2, ENCOR-TOP, CCNA_Study_Portal |
| Jun 1–5 | $61.08 | Sec+ + early ccna_wedge |
| Jun 6–21 | $88.51 | Mostly ccna_wedge_lab |
| Jun 24 | $9.63 | **Current Sec+ PBQ campaign** |
| **Jun 25** | **$24.58** | **12 clicks · overview export** |
| **Total** | **$280.26** | |

### Sec+ by day (same campaign family, mixed names)

| Date | Sec+ spend | Clicks |
|------|----------:|-------:|
| Jun 1 | $3.24 | 2 |
| Jun 2 | $27.62 | 14 |
| Jun 3 | $11.81 | 6 |
| Jun 4 | $10.47 | 7 |
| Jun 5 | $10.62 | 8 |
| Jun 8 | $10.55 | 5 |
| Jun 9 | $15.20 | 7 |
| Jun 10 | $16.92 | 9 |
| Jun 16 | $6.65 | 3 |
| **Jun 24** | **$9.63** | **4** |
| **Jun 25** | **$24.58** | **12** |

---

## Milestones (projected at $15/day, ccna paused)

| Date | Projected cumulative | Event |
|------|---------------------:|-------|
| Jun 25 (actual) | **$280.26** | Today |
| Jul 1 | ~$370 | Week 1 review |
| **~Jul 11** | **~$520** | **Promo threshold** |
| Jul 14 | ~$565 | 21-day conversion test ends |
| Jul 19 | ~$640 | Promo deadline |

---

## Update actuals weekly

1. Google Ads → **Billing → Billing activity** → export CSV (same format)
2. Paste daily **Account spend** into [[google-ads-spend-promo-2026.csv]]
3. Reconcile **Remaining to $500** with Google’s promo **“Check your progress”** widget

**Columns in CSV:**

- **Account spend USD** — all campaign rows (promo basis unless Google says otherwise)
- **Sec+ spend USD** — Security+ / SEC+ campaign names only
- **CCNA wedge USD** / **Other campaigns USD** — split for your records
- **Taxes and fees USD** — usually **not** promo-eligible; tracked separately

---

## Do not

- Raise budget to $20/day **only** for the promo — **$15/day is enough** at current pace
- Assume this CSV overrides Google’s promo meter — **widget is authoritative**
- Forget to **pause ccna_wedge_lab** if you want spend concentrated on Sec+ learning

[[README|← Sec+ Campaign]] · [[Sec+ Notes|Setup steps]]
