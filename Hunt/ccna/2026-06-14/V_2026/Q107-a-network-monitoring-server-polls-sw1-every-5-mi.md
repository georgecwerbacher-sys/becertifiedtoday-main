...
```text
SW1 clues show running-config | include snmp-server snmp-server community MON ro show logging | include Gi1/0/48 %LINK-3-UPDOWN: Interface Gi1/0/48, changed state to down %LINK-3-UPDOWN: Interface Gi1/0/48, changed state to up
```

A network monitoring server polls SW1 every 5 minutes. The NMS graphs interface counters and CPU normally, but it did not alert when uplink Gi1/0/48 went down for 2 minutes. Which action best addresses the supported root cause?
