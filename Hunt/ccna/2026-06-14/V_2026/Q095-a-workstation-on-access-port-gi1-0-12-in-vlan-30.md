...
Capture point Observed traffic Gi1/0/12 client port DHCP Discover from client MAC every 4 seconds Gi1/0/48 trunk VLAN 10 and VLAN 20 traffic seen; no VLAN 30 frames from
```

A workstation on access port Gi1/0/12 in VLAN 30 cannot obtain an IPv4 address by DHCP. The DHCP server is reached through trunk Gi1/0/48 toward the distribution switch. You run simultaneous packet captures for 30 seconds. the client MAC What is the best interpretation?
